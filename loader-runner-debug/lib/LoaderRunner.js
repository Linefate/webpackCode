...
function iteratePitchingLoaders(options, loaderContext, callback) {
    ...
    // loaderIndex 从 0 开始，表示从最左边的 loader 开始
	...
	loadLoader(currentLoaderObject, function(err) {
		...
		// 如果 pitch 返回结果，则反转方向开始 normal 阶段
		// 否则继续向右执行下一个 loader 的 pitch
		runSyncOrAsync(
			fn,
			...
			function(err) {
				...
				// ✅关键代码（熔断）：检查 pitch 是否返回值
				var hasArg = args.some(function (value) {
					return value !== undefined;
				});
				if (hasArg) {
                    // 如果 pitch 返回值，则回退 loaderIndex 并开始 normal 阶段
					loaderContext.loaderIndex--;
...
function processResource(options, loaderContext, callback) {
    ...
    // 设置 loaderIndex 为最后一个 loader（最右边）
...
function iterateNormalLoaders(options, loaderContext, args, callback) {
	...
	if (currentLoaderObject.normalExecuted) {
		// 递减 loaderIndex，向左移动到下一个 loader
		loaderContext.loaderIndex--;
