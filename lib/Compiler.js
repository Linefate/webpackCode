...
${other}`);
	...
	compile(callback) {
		...
		this.hooks.beforeCompile.callAsync(params, err => {
			...
			console.log("[Compiler] 调用 compile hook");
			this.hooks.compile.call(params); // 创建compilation

			// 2. 创建compilation对象
			console.log("[Compiler] 创建 compilation 对象");
			...
			// 3. 开始构建
			// 构建阶段开始
			console.log("[Compiler] 开始构建阶段 - make hook");
			this.hooks.make.callAsync(compilation, err => {
				logger.timeEnd("make hook");
				if (err) return callback(err);

				logger.time("finish make hook");
				console.log("[Compiler] 构建完成 - finishMake hook");
				this.hooks.finishMake.callAsync(compilation, err => {
					...
					process.nextTick(() => {
						...
						// 4. 完成构建
						debugger; // 构建完成
						console.log("[Compiler] 完成构建 - finish");
						compilation.finish(err => {
							...
							// 5. 封装构建结果
							debugger; // 封装阶段
							console.log("[Compiler] 开始封装 - seal");
							compilation.seal(err => {
								logger.timeEnd("seal compilation");
								if (err) return callback(err);

								logger.time("afterCompile hook");
								console.log("[Compiler] 编译后处理 - afterCompile");
